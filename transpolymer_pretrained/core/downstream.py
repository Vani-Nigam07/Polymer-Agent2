...
from transpolymer_pretrained.core.dataset import Downstream_Dataset, DataAugmentation
...
def train(model, optimizer, scheduler, loss_fn, train_dataloader, device):
    ...
    for batch in tqdm(train_dataloader, desc=f"Training Epoch", leave=False):
...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler, optimizer, scheduler, epoch):   
    ...
    with torch.no_grad():
        train_pred, train_true, test_pred, test_true = torch.tensor([]), torch.tensor([]), torch.tensor(
            []), torch.tensor([])       
        # for step, batch in enumerate(train_dataloader):
        for batch in tqdm(test_dataloader, desc=f"Evaluating Epoch", leave=False):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            prop = batch["prop"].to(device).float()
            outputs = model(input_ids, attention_mask).float()
            with torch.no_grad():
                ...
                train_true = torch.cat([train_true.to(device), prop.to(device)]) 
                        ...
                        # Move all concatenations to CPU
            # train_pred = torch.cat([train_pred, outputs.cpu()])
            # train_true = torch.cat([train_true, prop.cpu()])
        ...
        for step, batch in enumerate(test_dataloader):
            ...
            # test_pred = torch.cat([test_pred, outputs.cpu()])
            # test_true = torch.cat([test_true, prop.cpu()])
...
def _save_scaler(scaler, model_path): 
    """Save scaler next to the checkpoint, replacing .pt with .scaler.pkl""" 

    out = Path(model_path).with_suffix(".scaler.pkl")
    out.parent.mkdir(parents=True, exist_ok=True)

    joblib.dump(scaler, out)
    print(f"[scaler] saved -> {out}") 

# def test(model, loss_fn, test_dataloader, device, scaler, epoch, writer=None):
#     model.eval()
#     r2score = R2Score()
#     test_loss = 0.0    
#     all_preds = []
...
#             prop = prop.cpu()            # Inverse transform if scaler is used
#             outputs = torch.from_numpy(scaler.inverse_transform(outputs.numpy().reshape(-1, 1)))
#             prop = torch.from_numpy(scaler.inverse_transform(prop.numpy().reshape(-1, 1)))            
#             loss = loss_fn(outputs.squeeze(), prop.squeeze())
#             test_loss += loss.item() * outputs.size(0)           
#             all_preds.append(outputs)
#             all_labels.append(prop)    # Concatenate once after loop
#     all_preds = torch.cat(all_preds)
#     all_labels = torch.cat(all_labels)    
#     test_loss = test_loss / len(all_preds)
#     r2_test = r2score(all_preds, all_labels).item()   
#     print(f"Epoch {epoch}: Test RMSE = {np.sqrt(test_loss):.4f}, r^2 = {r2_test:.4f}")    
#     if writer:
#         writer.add_scalar("Loss/test", test_loss, epoch)
#         writer.add_scalar("r^2/test", r2_test, epoch)    
    ...
    if finetune_config['CV_flag']:
        ...
        for epoch in range(finetune_config['num_epochs']):
            ...
            if r2_test > best_r2:
                ...
                torch.save(state, finetune_config['best_model_path'])         # save the best model
                ## add Scaler.pkl
                _save_scaler(scaler,  finetune_config['best_model_path'])
            if count >= finetune_config['tolerance']:
                print("Early stop")
                if best_test_r2 == 0:
                    print("Poor performance with negative r^2")
...
if __name__ == "__main__":
    # optionally set this once
    CONFIG_DIR = '/home/vani/transpolymer/transpolymer_pretrained/configs'
    ...
    parser.add_argument(
        ...
        "--property",
        dest="property_name",
        default="PE_I",                       # default property
        help="Target property to fine-tune on, e.g. PE_I, OPV",
    )
    args = parser.parse_args()

    # Choose config file based on property
    cfg_path = os.path.join(
        CONFIG_DIR,
        f"config_{args.property_name}.yaml"
    )

    finetune_config = yaml.load(open(cfg_path, "r"), Loader=yaml.FullLoader)
    ...
    if finetune_config['model_indicator'] == 'pretrain':
        ...
        PretrainedModel = RobertaModel.from_pretrained(finetune_config['model_path'], local_files_only=True)
